...
import '../../common/assets/css/publications.css';
...
function App() {
    ...
    const loadData = () => {
        ...
        HTTP.get(Routes.api.frontend.publications, {
            isPrivate: false
        })
        .then(response => {
    ...
    return (
        <React.Fragment>
                    ...
                    <Row>
                        ...
                        <Col span={24} className="text-center">
                            <Row justify='center' gutter={32}>
                                {
                                    data.filter(publication => selectedCategory === null || (selectedCategory !== null && JSON.parse(publication.categories).includes(selectedCategory))).map((item, index) => (
                                        <Col
                                            key={index}
                                            xl={6}
                                            lg={6}
            ...
            </Row>
            {
                modalVisible && (
                    <PublicationPopup
                        title={selectedPublication ? selectedPublication.title : ''}
...
if (document.getElementById('react-publication-root')) {
   ReactDOM.render(
        <React.StrictMode>
            <App />
        </React.StrictMode>,
        document.getElementById('react-publication-root')
    ); 
}