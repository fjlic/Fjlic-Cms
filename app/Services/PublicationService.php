...
class PublicationService implements PublicationInterface
{
    /**
     * Eloquent instance
     *
     ...
     * @return array
     */
    private function processThumbnail(UploadedFile $file, $publication = null)
    {
        if ($publication) {
            //delete previous
            try {
                if (file_exists($publication->thumbnail)) {
                    unlink($publication->thumbnail);
                }
            } catch (\Throwable $th) {
                Log::error($th->getMessage());
            }
        ...
        try {
            ...
            $pathName = 'assets/common/img/publications/';
     ...
     * @param Publcation|null $publication
     * @return array
     */
    private function processImages(Array $fileArray, $publication = null)
    {
        if ($publication) {
            //delete previous
            try {
                $existingImages = json_decode($publication->images, true);
